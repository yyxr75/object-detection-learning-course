...
for i=1:size(signal(FRAME_BEGIN).radar_data_Static, 1)
    ...
    for frame_id = FRAME_BEGIN:FRAME_END
        ...
        radar_data_pre_apes(frame_id - FRAME_BEGIN + 1, :) = squeeze(radar_data_Static_(i,:));
        
    end
    % Capon计算角度
    radar_data_angle_range_Static(i,:) = APES_Alg(radar_data_pre_apes', angleFFTSize);
end
